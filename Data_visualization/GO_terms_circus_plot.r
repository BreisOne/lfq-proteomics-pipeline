#From dataframes generated in GO_Terms_analysis.R

circ <- circle_dat(FA.results2, Hela_df_clusPro1)
chord <- chord_dat(data = circ, genes = genes_logFC , process = Processes)
Custom_Gochord(chord, ribbon.col= brewer.pal (4, 'Set2'), process.label= 10, space = 0.02, gene.order = 'logFC', gene.space = 0.25, gene.size = 2.5)

####Custom chord function to adjust legend position and scale#######

Custom_Gochord <- function (data, title, space, gene.order, gene.size, gene.space, 
  ...
  g <- ggplot() + geom_polygon(data = df_process, aes(x, y, 
                               ...
                               color = "black") + geom_polygon(data = df_process, 
                                                               ...
                                                               alpha = 0.6, color = "black") + geom_point(aes(x = xpro, 
                                                                                                          ...
                                                                                                          data = df_texp) + guides(size = guide_legend("GO Terms", 
                                                                                                                                                       ncol = 1, byrow = T, override.aes = list(shape = 22, 
